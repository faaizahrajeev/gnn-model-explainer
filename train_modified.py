...
for i, node in enumerate(graph_case.nodes()):
    graph_case.nodes[node]['feature'] = features_case[:, i]  # (40,) per node

for i, node in enumerate(graph_control.nodes()):
    graph_control.nodes[node]['feature'] = features_control[:, i]  # (40,) per node
...
def prepare_data(case_graphs, control_graphs, args, max_nodes=0):
    """
    Prepare microbiome dataset for GNN training.
    
    Args:
        case_graphs (list): List of NetworkX graphs for case group.
        control_graphs (list): List of NetworkX graphs for control group.
        args: Training arguments.
        max_nodes (int): Maximum number of nodes in a graph.
    
    Returns:
        Data loaders and dataset properties.
    """
    
    # Combine case and control graphs into one dataset
    graphs = case_graphs + control_graphs
    
    # Assign labels directly to graph metadata (since GraphSampler reads from G.graph['label'])
    for G in case_graphs:
        G.graph["label"] = 1  # Case group labeled as 1
    for G in control_graphs:
        G.graph["label"] = 0  # Control group labeled as 0
    ...
    val_idx = train_idx + int(len(graphs) * args.val_ratio)  # Corrected
    train_graphs = graphs[:train_idx]
    val_graphs = graphs[train_idx:val_idx]
    test_graphs = graphs[val_idx:]

    print(f"Train: {len(train_graphs)}, Val: {len(val_graphs)}, Test: {len(test_graphs)}")
    print(f"Total graphs: {len(graphs)}, Total edges: {sum(G.number_of_edges() for G in graphs)}")

    # Create GraphSampler datasets
    train_dataset = graph_utils.GraphSampler(train_graphs, features=args.feature_type, max_num_nodes=max_nodes)
    val_dataset = graph_utils.GraphSampler(val_graphs, features=args.feature_type, max_num_nodes=max_nodes)
    test_dataset = graph_utils.GraphSampler(test_graphs, features=args.feature_type, max_num_nodes=max_nodes)
    ...
    return train_loader, val_loader, test_loader, train_dataset.max_num_nodes, train_dataset.feat_dim, train_dataset.assign_feat_dim
